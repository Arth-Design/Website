...
if( ! class_exists('RatioEdgeSidebar') ){
	...
	class RatioEdgeSidebar{
		...
		function load_assets(){
			...
			wp_enqueue_script('ratio-edge-sidebar' , EDGE_ROOT . '/framework/admin/assets/js/edgtf-sidebar.js');
			wp_enqueue_style( 'ratio-edge-sidebar' , EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css');
		}
		...
		function delete_sidebar_area(){
			...
			if(!empty($_POST['name'])){
				...
				if(($key = array_search($name, $this->sidebars)) !== false){
					...
					update_option($this->stored, $this->sidebars);
					echo esc_html__( "sidebar-deleted", 'ratio' );
				}
			}
