...
if(!function_exists('ratio_edge_custom_breadcrumbs')) {
    ...
    function ratio_edge_custom_breadcrumbs() {
        ...
        if (is_home() && !is_front_page()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.ratio_edge_get_inline_style($bread_style).' href="' . esc_url($homeLink) . '">' . $home . '</a>' . $delimiter . ' <a '.ratio_edge_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.ratio_edge_get_inline_style($bread_style).' href="' . esc_url($homeLink) . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.ratio_edge_get_inline_style($bread_style).' href="' . esc_url($homeLink) . '">' . $home . '</a>' . $delimiter;
            ...
            if ( is_category() || ratio_edge_is_product_category()) {
                ...
                $output .= $before . esc_html__('Error 404','ratio') . $after;
            } elseif(function_exists("is_woocommerce") && is_shop()){
                $output .= $before .  get_the_title( ratio_edge_get_woo_shop_page_id() ) . $after;
            }
