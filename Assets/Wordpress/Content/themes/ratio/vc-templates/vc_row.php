...
if($video == 'show_video'){
	...
	$video_output .='<object width="320" height="240" type="application/x-shockwave-flash" data="'.EDGE_ASSETS_ROOT.'/js/flashmediaelement.swf">';
		...
		if(!empty($video_mp4)) {
			$video_output .= '<param name="flashvars" value="controls=true&amp;file=' . esc_url($video_mp4) . '" />';
		}
	if($v_image) {
		$video_output .= '<img ' . ratio_edge_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="'.esc_attr__('No video playback capabilities','ratio').'" alt="'.esc_attr__('video thumb','ratio').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
...
echo ratio_edge_get_module_part( $output );