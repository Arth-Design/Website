	...
	private function getSocialNetworkShareLink($net, $image) {
		...
		switch ($net) {
			...
			case 'twitter':
				...
				$twitter_via = (ratio_edge_options()->getOptionValue('twitter_via') !== '') ? esc_html__( ' via ', 'edge-cpt') . ratio_edge_options()->getOptionValue('twitter_via') . ' ' : '';
			...
			case 'tumblr':
				...
				break;
			case 'pinterest':
				$link = 'popUp=window.open(\'https://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . ratio_edge_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
				break;
			case 'vk':
