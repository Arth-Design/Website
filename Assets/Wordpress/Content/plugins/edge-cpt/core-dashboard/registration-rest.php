...
if( class_exists( 'RatioCoreDashboardRestAPI' ) ) {
	class RatioCoreRestAPIRegistrationPurchaseCode extends RatioCoreDashboardRestAPI {
		...
		public function register_rest_api_route() {
			...
			register_rest_route( $this->get_namespace(), $this->get_route(), array(
				array(
                    ...
                    'callback' => array(RatioCoreDashboard::get_instance(), 'purchase_code_registration'),
					...
					'args'     => array(
						'options' => array(
							...
							'description'       => esc_html__( 'Options data is array with parameters', 'ratio-core' )
						)
					)
				)
			) );
	...
	RatioCoreRestAPIRegistrationPurchaseCode::get_instance();
}